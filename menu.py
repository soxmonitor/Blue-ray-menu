...
menu_title = "胆大党01-08"  # 修改这里的内容来更改菜单标题
...
def capture_frame(video_file):
    ...
    cap.set(cv2.CAP_PROP_POS_FRAMES, 480)  # 跳转到第192帧
    ret, frame = cap.read()
    if ret:
...
def play_video(video_file):
    # 通过列表而非字符串方式调用 Popen，确保带空格的路径也能被正确解析
...
class VideoMenuApp:
    ...
    def create_menu(self):
        ...
        for i in range(start_index, end_index):
            ...
            # 截断并去除扩展名，仅用于显示
            short_title = shorten_title(self.root, video_name, max_width=200)
